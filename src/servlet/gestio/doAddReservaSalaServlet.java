...
/**
 * Servlet implementation class doAddReservaSalaServlet
 */
@WebServlet("/doAddReservaSala")
public class doAddReservaSalaServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public doAddReservaSalaServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    String errorString = null;	
   		if (cal.get(Calendar.DAY_OF_WEEK) == dia && cal.get(Calendar.WEEK_OF_YEAR) == setmana) {
			if (CalendarCore.potReservar(conn, idUsuari, "sala", setmana, dia, year, horaIni, horaFi, false)) { 
				CalendarCore.reservar(conn, idUsuari, "sala", setmana, dia, year, horaIni, horaFi, motiu);
			} else {
				errorString = "ocupat";
			}
		}
		else if (CalendarCore.potReservar(conn, idUsuari, "sala", setmana, dia, year, horaIni, horaFi, true)) {
			CalendarCore.reservar(conn, idUsuari, "sala", setmana, dia, year, horaIni, horaFi, motiu);
		} else {
			errorString = "ocupat";
		}	          	
