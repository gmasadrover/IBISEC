...
public class FacturaCore {
	...
	private static Factura initFactura(Connection conn, ResultSet rs, boolean ambDocuments) {	
		...
		try {
			...
			if (ambDocuments) {
				factura.setFactura(getArxiu(conn, rs.getString("idfactura")));
				factura.setTotsDocumentsFactura(getTotsArxiu(conn, rs.getString("idfactura")));
				factura.setAltres(getArxiusAltres(conn, rs.getString("idfactura")));
			}
	...
	private static Factura initCertificacio(Connection conn, ResultSet rs, boolean ambDocuments) {	
		Factura factura = new Factura();
		try {
			...
			factura.setUsuariCreador(UsuariCore.findUsuariByID(conn, rs.getInt("usucre")));
			if (ambDocuments) {
				factura.setCertificacions(getArxiuCertificacio(conn, ActuacioCore.findActuacio(conn, rs.getString("idactuacio")).getIdIncidencia(), rs.getString("idactuacio"), rs.getString("idinforme"), rs.getString("nifproveidor"), rs.getString("idcertificacio"), false));
				factura.setRelaciovalorada(getArxiuRelacioValorada(conn, ActuacioCore.findActuacio(conn, rs.getString("idactuacio")).getIdIncidencia(), rs.getString("idactuacio"), rs.getString("idinforme"), rs.getString("nifproveidor"), rs.getString("idcertificacio"), false));
				factura.setAltres(getArxiusAltres(conn, rs.getString("idcertificacio")));
			}
			factura.setAnulada(rs.getBoolean("anulada"));
			factura.setMotiuAnulada(rs.getString("motiuanulada"));
	...
	public static void saveArxiuCertificacio(String idIncidencia, String idActuacio, String idInforme, String cifEmpresa, String idCertificacio, List<Fitxer> fitxers, Connection conn, int idUsuari) {
		if (!fitxers.isEmpty()) {
	...
	public static void saveArxiuRelacioValorada(String idIncidencia, String idActuacio, String idInforme, String cifEmpresa, String idCertificacio, List<Fitxer> fitxers, Connection conn, int idUsuari) {
		if (!fitxers.isEmpty()) {			
			String fileName = "";
			// Crear directoris si no existeixen			
			String ruta = ConfiguracioCore.getConfiguracio(conn).getRutaBaseDocumentacio();
			...
			tmpFile = new File(ruta + "/documents/Certificacions/" + idCertificacio + "/RelacioValorada");
			if (!tmpFile.exists()) {
				tmpFile.mkdir();
			}
			fileName = ruta + "/documents/Certificacions/" + idCertificacio + "/RelacioValorada/";
	        String noms = "";
			for(int i=0;i<fitxers.size();i++){		        	
	            Fitxer fitxer = (Fitxer) fitxers.get(i);	            
	            if (fitxer.getFitxer().getName() != "") {		            	
	            	...
	            	Fitxers.guardarRegistreFitxer(conn, noms, fileName  + "/" + fitxer.getFitxer().getName(), idUsuari);
	            } 
	        }
	...
	public static void saveArxiu(String idIncidencia, String idActuacio, String idInforme, String cifEmpresa, String idFactura, List<Fitxer> fitxers, Connection conn, int idUsuari) {
	...
	public static void saveArxiuAltres(String idIncidencia, String idActuacio, String idInforme, String cifEmpresa, String idFactura, List<Fitxer> fitxers, Connection conn, int idUsuari) {
		if (fitxers != null && fitxers.size() > 0) {
			String fileName = "";
			// Crear directoris si no existeixen
	...
	public static List<Fitxer> getArxiuCertificacio(Connection conn, String idIncidencia, String idActuacio, String idInforme, String cifEmpresa, String idCertificacio, boolean readFirma) {
		List<Fitxer> factura = new ArrayList<Fitxer>();		
		String ruta =  ConfiguracioCore.getConfiguracio(conn).getRutaBaseDocumentacio();
		if (idIncidencia == null) idIncidencia = "-1";
		factura =  utils.Fitxers.ObtenirFitxers(ruta + "/documents/Certificacions/" + idCertificacio + "/", true);
		return factura;
	}
	
	public static List<Fitxer> getArxiuRelacioValorada(Connection conn, String idIncidencia, String idActuacio, String idInforme, String cifEmpresa, String idCertificacio, boolean readFirma) {
		List<Fitxer> factura = new ArrayList<Fitxer>();		
		String ruta =  ConfiguracioCore.getConfiguracio(conn).getRutaBaseDocumentacio();
		if (idIncidencia == null) idIncidencia = "-1";
		factura =  utils.Fitxers.ObtenirTotsFitxers(ruta + "/documents/Certificacions/" + idCertificacio + "/RelacioValorada", "relació");
		return factura;
	}
	
	
	public static Fitxer getArxiu(Connection conn, String idFactura) {
	...
	public static List<Fitxer> getTotsArxiu(Connection conn, String idFactura) {
	...
	public static List<Fitxer> getArxiusAltres(Connection conn, String idFactura) {
		List<Fitxer> altres = new ArrayList<Fitxer>();
	...
	public static void newFactura(Connection conn, Factura factura, int idUsuari) {
	...
	public static void newCertificacio(Connection conn, Factura certificacio, int idUsuari) {
	...
	public static void modificarFactura(Connection conn, Factura factura, int idUsuari) {
	...
	public static void modificarCertificacio(Connection conn, Factura certificacio, int idUsuari) {
	...
	public static Factura getFactura(Connection conn, String idFactura) {
		Factura factura = new Factura();
	...
	public static Factura getCertificacio(Connection conn, String idCertificacio) {
		Factura factura = new Factura();
	...
	public static List<Factura> getFacturesNoAssociades(Connection conn) {
		...
		String sql = "SELECT " + CAMPS_FACTURA
		 		...
		 		+ " WHERE f.anulada = false AND f.dataenviatcomptabilitat IS NULL AND f.idinforme = '-1' AND f.datadescarregadafactura IS NULL";
		PreparedStatement pstm;
		try {
			pstm = conn.prepareStatement(sql);
			ResultSet rs = pstm.executeQuery();
	...
	public static List<Factura> advancedSearchFactures(Connection conn, Date dataInici, Date dataFi, String estatFactura) {
		List<Factura> factures = new ArrayList<Factura>();
		...
		try {
			...
			while (rs.next()) {
				...
				factura.setAnulada(rs.getBoolean("anulada"));
				factura.setMotiuAnulada(rs.getString("motiuanulada"));
				factura.setFactura(FacturaCore.getArxiu(conn, rs.getString("idfactura")));
	...
	public static List<Factura> advancedSearchCertificacions(Connection conn, Date dataInici, Date dataFi, String idCentre) {
		List<Factura> factures = new ArrayList<Factura>();
	...
	public static List<Factura> searchFactures(Connection conn, Date dataInici, Date dataFi) {
		List<Factura> factures = new ArrayList<Factura>();
		String sql = "SELECT " + SQL_CAMPS
			 		+ " FROM public.tbl_factures"
	...
	public static List<Factura> getFacturesActuacio(Connection conn, String idActuacio)  {
		List<Factura> factures = new ArrayList<Factura>();
		String sql = "SELECT " + SQL_CAMPS
			 		+ " FROM public.tbl_factures"
			 		+ " WHERE idactuacio = ?";	 
	...
	public static List<Factura> getFacturesInforme(Connection conn, String idInforme) {
		List<Factura> factures = new ArrayList<Factura>();
		String sql = "SELECT " + SQL_CAMPS
			 		+ " FROM public.tbl_factures"
			 		+ " WHERE idinforme = ?";	 
	...
	public static double getTotalFacturatInforme(Connection conn, String idInforme) {
	...
	public static List<Factura> getCertificacionsInforme(Connection conn, String idInforme) {
		List<Factura> factures = new ArrayList<Factura>();
		String sql = "SELECT " + SQL_CAMPS_CERT
			 		+ " FROM public.tbl_certificacions"
			 		+ " WHERE idinforme = ?";	 
	...
	public static String getNewCodeCert(Connection conn) {
		String code = "1";
		Calendar now = Calendar.getInstance();
		int year = now.get(Calendar.YEAR);
		String yearInString = String.valueOf(year);
	...
	public static String getCodeCertAntic(Connection conn) {
		String code = "1";
		Calendar now = Calendar.getInstance();
		int year = now.get(Calendar.YEAR);
		String yearInString = String.valueOf(year);
		String sql = "SELECT idcertificacio, datacertificacio"
					+ " FROM public.tbl_certificacions"
					+ " WHERE idcertificacio like '%" + yearInString + "-ANTIC%'"
					+ " ORDER BY idcertificacio DESC, datacre DESC LIMIT 1;";	
		PreparedStatement pstm;
		try {
			pstm = conn.prepareStatement(sql);
	...
	public static String getCodeFacturaAntic(Connection conn) {
		String code = "1";
		Calendar now = Calendar.getInstance();
		int year = now.get(Calendar.YEAR);
		String yearInString = String.valueOf(year);
		String sql = "SELECT idfactura, datafactura"
					+ " FROM public.tbl_factures"
					+ " WHERE idfactura like '%" + yearInString + "-FANTIC%'"
					+ " ORDER BY idfactura DESC, datacre DESC LIMIT 1;";	
		PreparedStatement pstm;
		try {
			pstm = conn.prepareStatement(sql);
	...
	public static void descarregada(Connection conn, String idFactura) {
		String sql = "UPDATE  public.tbl_factures"
	...
	public static void anular(Connection conn, String idFactura, String motiu) {
		String sql = "UPDATE  public.tbl_factures"
	...
	public static void anularCertificacio(Connection conn, String idFactura, String motiu) {
		String sql = "UPDATE  public.tbl_certificacions"
				+ " SET anulada = true, motiuanulada = ?"
		 		+ " WHERE idcertificacio = ?";	 
		PreparedStatement pstm;
	...
	public static List<Factura> getFacturesConformadesPend(Connection conn) {
	...
	public static List<Factura> getFacturesNoConformadesPend(Connection conn) {
		List<Factura> facturesList = new ArrayList<Factura>();
		String sql = "SELECT " + SQL_CAMPS
				+ " FROM public.tbl_factures"
				+ " WHERE anulada = false AND idinforme != '-1' AND dataconformador IS NULL AND dataenviatcomptabilitat IS NULL AND datadescarregadafactura IS NULL AND dataentrada < ?";	
		Calendar cal = Calendar.getInstance();	
		cal.add(Calendar.DATE, -14);
		Date quinzeDies = cal.getTime();
		PreparedStatement pstm;
		try {		
			pstm = conn.prepareStatement(sql);
			pstm.setDate(1, new java.sql.Date(quinzeDies.getTime()));
			ResultSet rs = pstm.executeQuery();
			while (rs.next()) {			
				facturesList.add(initFactura(conn, rs, true));
			}
	...
	public static List<Factura> getFacturesPerInforme(Connection conn, String nif, Date dataIni, Date dataFi) {
		List<Factura> facturesList = new ArrayList<Factura>();
